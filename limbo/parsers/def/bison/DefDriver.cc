...
namespace DefParser
{
	...
	{
		cout << "Parsing DEF file in parse_file " << endl;
						 ...
						 int repeatx, int repeaty, int stepx, int stepy)
	{
		...
		m_row.repeat[0] = repeatx;
		m_row.repeat[1] = repeaty;
		m_row.step[0] = stepx;
		m_row.step[1] = stepy;
	...
	void Driver::component_cbk_position(string &status, int originx, int originy, string &orient)
	{
		...
		m_comp.origin[0] = originx;
		m_comp.origin[1] = originy;
	...
	void Driver::pin_cbk_position(string &status, int originx, int originy, string &orient)
	{
		...
		m_pin.origin[0] = originx;
		m_pin.origin[1] = originy;
